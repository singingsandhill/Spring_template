...
public class JwtUsernamePasswordAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        LoginDTO login = LoginDTO.of(request);  // 요청 BODY의 JSON에서 username, password  LoginDTO
        UsernamePasswordAuthenticationToken authenticationToken =
                new UsernamePasswordAuthenticationToken(login.getUsername(), login.getPassword()); // 인증 토큰(UsernamePasswordAuthenticationToken) 구성

        return getAuthenticationManager().authenticate(authenticationToken);        // AuthenticationManager에게 인증 요청
    }
}
